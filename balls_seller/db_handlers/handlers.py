...
def get_ordered_blowup_balls_from_DB(nickname: int):
    connection = sqlite3.Connection(os.path.join('db', 'balls_seller.sqlite'))
    cursor = connection.cursor()
    cursor.execute(f"SELECT Orders.amount, Orders.notes, BlowUp_Balls.price FROM Orders, BlowUp_Balls WHERE Orders.nickname=={nickname} and Orders.type=='Blow up'")
    ordered_common_balls = cursor.fetchall()
    connection.close()
    return ordered_common_balls
...
def get_shaped_pictures_from_DB(shaped_ball_type=None, shaped_ball_subtype=None):
    ...
    if shaped_ball_type is None and shaped_ball_subtype is None:
        ...
        cursor.execute(f"SELECT picture, amount FROM Shaped_Balls "
                       f"where type == '{shaped_ball_type}' "
        ...
        for i in range(len(pictures_names)):
            pictures_names_and_amounts[i][0] = gen_picture_path(pictures_names_and_amounts[i][0], balloon_type="shaped")
        connection.close()
